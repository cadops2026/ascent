// Vendored copy of src/lib/finance/networth.ts for the Deno edge runtime.
// Logic is IDENTICAL to the app's engine (invariant #1 — one source of truth);
// only the imports differ (DB-row types come from ./types.ts, .ts extensions).
// Keep in sync if the app's version changes.
import type { Holding, RealEstate, Liability, QuoteMap } from './types.ts'
import { amortize } from './amortization.ts'

export type { QuoteMap } from './types.ts'
...
export function computeBalanceSheet(
  ...
  return { totalAssets, totalLiabilities, netWorth, investable, residenceValue, byClass, pendingQuotes }
}
